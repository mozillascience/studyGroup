...
When you work with many datasets of different versions, it is important to properly organise your files on your computer. It is useful to have a folder where you place all your projects.
...
We can use the `read_csv` function to load a CSV file, assuming the file uses `,` to separate the different values. If the file uses `;` to separate the values we use `read_csv2`. The following code reads the `train.csv` file from the current directory and stores the resulting `data.frame` in the variable `df.raw`. NOTE: In R many use `<-` to store something into a variable, but you can also use `=` for this if you want.
