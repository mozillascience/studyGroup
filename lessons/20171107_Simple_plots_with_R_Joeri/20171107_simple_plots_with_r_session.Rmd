...
You can find more information [here](https://www.tidyverse.org). To install the package you can either enter `install.packages("tidyverse")` in the Console (the bottom panel in RStudio) or you can open the "Packages" tab in the right panel, then click the "Install" button and type "tidyverse" in the dialog.

After installing the package you will need to load it. It best to write this command in the notebook as RStudio will then load the library it every time you execute the document (for example when you 'knit' or 'preview' it):
...
```{r}
df.raw <- read_csv("data/train.csv")
```
...
```{r}
ggplot(df.raw, aes(x=Pclass, y=Fare)) +
  geom_point() +
  labs(x="Passenger Class", y="Fare", title="My First Plot in R")
```

```{r}
ggplot(df.raw, aes(x=Pclass, y=Fare, colour=Sex)) +
  geom_point() +
  labs(x="Passenger Class", y="Fare", title="My First Plot in R")
```
...
df.raw.f <- df.raw %>%
  filter(Fare < 500) %>%
  mutate( Survived = ifelse( Survived == 0, "Not Survived", "Survived" ) ) 

df.raw.f %>%
  ggplot(aes(x=Sex, y=Fare)) +
  facet_wrap( ~ Survived, labeller = "label_both" ) +
  geom_violin()
```
