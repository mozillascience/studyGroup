...
Filter_Example4 <- filter(Alaska,common_name == "Clams",
                            reporting_status =="Unreported") # Will give me all clams that are unreported
...
### Rename
The `rename` function is another "self explainatory" it allows you to rename the columns
...
=======
###Joining Data with dplyr
>>>>>>> 7e3de76fb81ad0e0f103bc0ac3937709fca1c5be
...
Lets us know what variables from one dataset are not present in some other dataset
...
#Lets assume we want to know how many species are fished in Alaska and not in the continental US
...
#Bothe Data 1 have two columns from witch one is "common_name". In the case of Data 1 the second column is "functional_group" and in the case of Data2 its "fishing_sector"

Inner_Example <- inner_join(Data1, Data2, by="common_name")

#The result will be a data-set with the "common_name", "functional_group" and "fishing_sector"
...
### Gather
The `gather` function allows us to convert long data intoo short format. THis is specifically helpfull for plotting since it will allow you to set categories to data.

Note: The `spread`function is exactlly the oposite to `gather` and has the same structure



```{r gather, echo=t, eval=T}

# For example, if you want to have a divission between scientific and common name to plot the tonnes you'll do something like this:
...
These functions are used to unite or spread dates on a dataset
