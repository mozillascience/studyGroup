...
author: Vicki Zhang, borrowed from Ahmed Hasan, James Santangelo, and Joel Ostblom
output: pdf_document
---
...
from data frames. The name `ggplot2` comes from its inspiration, the book [_A
Grammar of Graphics_](https://www.springer.com/gp/book/9780387245447), and the
main goal is to allow coders to distill complex data structure and express their
desired graphical outcome in a concise manner instead of telling the computer
every detail about what should happen. For example, you would say "colour my data
by species" instead of "go through this data frame and plot any observations of
species1 in blue, any observations of species2 in red, and so on". Thanks to
this functional way of interfacing with data, only minimal changes are required
if the underlying data change. This helps in thinking about the data and creating
...
explain this pattern. colouring the data points according to a categorical
...
Adding the colour aesthetic we had previously will cause lines to be fit to each individual group:
...
To still fit a line across the entire dataset, specify the colour aesthetic _within_ `geom_point`, and not globally.
...
Let's create a graph object with all of our variables plotted nicely, and then we can customize the non-data components of the plot.
```{r}
# create graph object
...
We can change the axis titles by scaling them.

```{r}
p +
  labs(x = "Carat", y = "Price (USD)", colour = "Clarity", # capitalize axis labels, indicate currency
  title = "Price of diamonds for different carats and clarity") # add title
```

We can customize the appearance more by passing some _theme_ statements[^theme] to `ggplot`.
```{r}
p + 
  labs(x = "Carat", y = "Price (USD)", colour = "Clarity", title = "Price of diamonds for different carats and clarity") +
  theme(plot.title = element_text(family = "Helvetica",
        ...
        panel.background = element_rect(fill = "lightblue"), # only plot area
        plot.background = element_rect(fill = "grey", # plot is for the entire plot (very confusing)
                                       colour = "black"))
```
...
The above plot is probably the most atrocious figure I have ever made. Thankfully, `ggplot2` also comes preloaded with a host of pre-made themes - try them out below. I like to use the classic `theme_classic` because it also gets rid of grid lines - it looks so clean!
...
More fun stuff you can do on  `ggplot`

* using `patchwork` to combine plots
* changing all aspects of the graph by using `scale_`, such as the x-axis, the
y-axis, the colour, the legend, etc.
* changing non-graphical components of the plot using `theme` and `element_`
arguments
* using arguments such as `jitter` and `position_dodge` to increase readability
of plots (e.g., avoid overlaps, etc.)
