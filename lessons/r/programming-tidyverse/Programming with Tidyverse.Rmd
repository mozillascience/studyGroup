...
```
You'll have gotten an error. The problem is that `group_by` is an action verb and not a selection verb so it needs a bit of help understanding inputs like all_of. Use `across` in addition to `all_of` instead to fix the problem.
```{r}
#TIDYING IN A FUNCTION, ATTEMPT 4#
diamondsFunction <- function(diamondColor, diamondVar1,diamondVar2){
  normalizedDiamonds <- diamonds%>%
    filter(color==diamondColor)%>%
    group_by(across(all_of(diamondVar1)))%>%
    mutate(normalizedPrice=(price-mean(price))/sd(price))
