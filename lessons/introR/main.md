...
You also **don't need to read all of this** for the session.  It's
more of a **resource/reference**.
...
I should mention a quick caveat.  R is a programming
language... however it works very differently from other
general-purpose programming languages ie. Python (it was developed by
statisticians after all!).  As such, programming in R can be not as
intuitive, powerful, or easy as it may be in Python (though it can be
done).  If your work involves a lot of programming, I would recommend
Python as your main tool.  *However*, it never hurts to learn more
than one language, especially as R is great for the data analysis and
plotting.
...
I'm using R Markdown to show how to write up documents and since
getting the data into an analyzable form is the hardest part of an
analysis, I'm using packages specific to that task.
...
code into a generated `.md` file.  This is a incredible (recent)
...
You'll need to import your data into R to be used.  I'm assuming the
data is already cleaned and ready for analysis.  If at any time you
need help with a command, use the `?` command, appended with the
command of interest (eg. `?names` or `?write.csv`).
...
of each of your columns.  `class()` is like `str()` but only shows the
top level name of the object, so eg. while a data.frame contains
...
Data wrangling is a bit tedious in base R.  So I'm showing two
packages designed to be this easier.  Load packages by using the
`library()` function.  `dplyr` comes via the `magrittr` package with a
`%>%` pipe function, which works similar to how the Bash shell `|`
pipe works.  It end of each pipe takes the output from the right hand
side of the pipe, just like how a plumbing pipe works for water.  The
four lines of code below using `tbl_df` are all the same.  The `.`
object represents the output from the pipe.  `tbl_df` makes the object
...
Again, these next lines are the same. `select` does as it says it
does: select the column from the dataset.
...
command.  There is no end to how long a chain can be.  `arrange` below
arranges (sorts, orders, rearranges, etc) the column Education in
ascending order. `mutate` creates a new column.
...
To get the data into a nicer/more analyable format, you can use the
`tidyr` package.  See what `gather` does.  Then see what `spread`
does.  Note that you can remove a column by having a minus `-` sign
infront of a variable when you use `select`.
...
makes it so futher analyses or operations work on the groups.
...
`eval = FALSE`, which means don't run this code chunk.
